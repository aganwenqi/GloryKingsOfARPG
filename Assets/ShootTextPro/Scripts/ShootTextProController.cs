...
public class ShootTextProController : MonoBehaviour
{
    ...
    private readonly string crit = "_crit";
    ...
    public Transform ShootTextCanvas
    {
        get
        {
            ...
            {
                shootTextCanvas = GameObject.Find("FlyCanvas").transform;
            }
            if (shootTextCanvas == null)
            {
    ...
    private void Initialized()
    {
        ...
        //暴击图片
        numberDic.Add(TextAnimationType.Normal.ToString() + crit, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Normal.ToString() + crit));
        numberDic.Add(TextAnimationType.Burst.ToString() + crit, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Burst.ToString() + crit));
    ...
    private void BuildNumber(ShootTextInfo shootTextInfo, Transform parent)
    {
        ...
        if (numberOperator != '+' && numberOperator != '-')
        {
            numberObj = Instantiate(numberDic[animationType + "_" + tempNumber]);
